...
dataset = "AWA1"
class_embed_dim = 85
Model_GAN_path = "/data3/huangmeixue/ZSL_GAN/Model_CLSWGAN/"+dataset+"/CLSW_GAN.pth"
PREFINE_CLASS_path = "/data3/huangmeixue/ZSL_GAN/Model_CLSWGAN/"+dataset+"/prefine_class.pth"
zsl_classifier_path = "/data3/huangmeixue/ZSL_GAN/Model_CLSWGAN/"+dataset+"/zsl_classifier.pth"
gzsl_classifier_path = "/data3/huangmeixue/ZSL_GAN/Model_CLSWGAN/"+dataset+"/gzsl_classifier.pth"
...
def next_batch(data,batch_size):
    ...
    batch_feature = data[idx]['feature']
    batch_label = data[idx]['label']
    batch_att = data[idx]['attribute']
...
def train(data):
    ...
    for step in range(0, len(data), BATCH_SIZE):
        ...
        for i in range(n_critic):
            feature, label, attribute = next_batch(data,BATCH_SIZE)
        ...
        # classification loss
        classificationLogits = Classifier(gen_img_feature)
        classificationLoss = classification_loss(classificationLogits,label)

        g_cls_loss = g_loss + class_weight * classificationLoss

        g_cls_loss.backward()
        optimizer_G.step()

        print(
            "[Epoch %d/%d] [Batch %d/%d] [D loss: %f] [W loss: %f] [Grad loss: %f] [G_cls loss: %f] [G loss: %f] [CLS loss: %f]"
            % (epoch+1, EPOCH, int(step/BATCH_SIZE), int(len(data)/BATCH_SIZE), d_loss.item(), Wasserstein_D.item(), gradient_penalty.item(), g_cls_loss.item(), g_loss.item(), classificationLoss.item())
        )
    return g_cls_loss,d_loss
...
def save_model(epoch,g_net,d_net,g_loss,d_loss,model_path):
    print('===> Saving CLSW_GAN GAN Model...')
    state = {
...
class ClassInfoData(Data.Dataset): #继承Dataset
    def __init__(self,features,attributes,labels): #__init__是初始化该类的一些基础参数
    ...
    def __getitem__(self,index):#根据索引index返回dataset[index]
        ...
        sample = {'feature':feature,'attribute':attribute,'label':label}#根据图片和标签创建字典
        return sample #返回该样本
...
############################# Train GAN model ###############################
        ...
        labels_train = torch.LongTensor(map_label(data.labels_train))
        train_data = ClassInfoData(features_train,attributes_train,labels_train)
        ...
        class_num = len(np.unique(np.where(data.labels_train.astype(int)==1)[1]))
        Classifier = SoftMax_Classifier(feature_dim,class_num)
        ...
        if os.path.exists(PREFINE_CLASS_path):
        	cls_checkpoint = torch.load(PREFINE_CLASS_path)
        	print("===> Loading Pre_Class Model... Start Epoch:{}".format(cls_checkpoint['epoch']))
        	Classifier.load_state_dict(cls_checkpoint['state_dict'])
        # freeze the classifier during the optimization
        for p in Classifier.parameters(): # set requires_grad to False
            p.requires_grad = False

        for epoch in range(EPOCH):
            g_cls_loss,d_loss = train(train_data)
            save_model(epoch,generator,discriminator,g_cls_loss,d_loss,Model_GAN_path)
...
########## generate image features and classification evalution #########
        ...
        print("===> Loading CLSW_GAN Model... Start Epoch:{}".format(checkpoint['epoch']))
        ...
        if validation:
            ...
            cls = classifier.Classifier(gen_features,gen_labels,data,zsl_classifier_path,lr=0.0001,batch_size=100,epoch=50,validation=True,generalized=False)
            ...
            if generalized:
                ...
                cls = classifier.Classifier(features_train,labels_train,data,gzsl_classifier_path,lr=0.0001,batch_size=100,epoch=50,validation=False,generalized=True)
                ...
                gen_labels = map_label(gen_labels.astype(int))
                cls = classifier.Classifier(gen_features,gen_labels,data,zsl_classifier_path,lr=0.0001,batch_size=100,epoch=50,validation=False,generalized=False)
                unseen_acc = cls.unseen_acc